...
function getTypeBadge(type: string): { label: string; color: string } | null {
  switch (type) {
    case 'announcement': return { label: 'Announcement', color: 'bg-indigo-100 text-indigo-700' };
    case 'assignment': return { label: 'Assignment', color: 'bg-amber-100 text-amber-700' };
    case 'material': return { label: 'Material', color: 'bg-emerald-100 text-emerald-700' };
    default: return null;
  }
}
...
export default function PostCard({ post, index, onAddComment }: PostCardProps) {
  ...
  const authorInitial = getInitials(authorName);
      ...
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <motion.div
            ...
            className="w-10 h-10 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white text-sm font-semibold flex-shrink-0"
          >
            {authorInitial}
          </motion.div>
          <div>
            <div className="flex items-center gap-2">
              ...
              {post.pinned && <Pin className="w-3 h-3 text-indigo-500" aria-label="Pinned post" />}
            </div>
            <div className="flex items-center gap-2 mt-0.5">
              <RelativeTime date={post.createdAt} className="text-xs text-gray-500" />
              {badge && (

...
export default function CommentSection({ comments, onAddComment }: CommentSectionProps) {
  ...
  const isCommentDisabled = !newComment.trim();
  ...
  return (
    <div className="mt-4">
      ...
      >
        ...
        {comments.length === 0 ? 'Comment' : `${comments.length} ${comments.length === 1 ? 'comment' : 'comments'}`}
      </button>
            ...
            <div className="pt-4 mt-3 border-t border-gray-100 space-y-3">
              <AnimatePresence mode="popLayout">
                {comments.map((comment) => {
                  ...
                  const cName = commentAuthor?.name || 'Unknown';
                  return (
                    <motion.div
                      key={comment.id}
                      layout
                    ...
                    >
                      <div
                        className={`w-7 h-7 mt-0.5 rounded-full bg-gradient-to-br ${getAvatarColor(cName)} flex items-center justify-center text-white text-[10px] font-semibold flex-shrink-0`}
                      >
                        {getInitials(cName)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="bg-gray-50 rounded-xl px-3 py-2">
                          <div className="flex items-center justify-between gap-2">
                            ...
                            <RelativeTime date={comment.createdAt} className="text-[10px] text-gray-400 flex-shrink-0" />
                          </div>
              ...
              <div className="flex items-start gap-2.5 pt-1">
                ...
                <div className="flex-1 flex items-end gap-2">
                  ...
                  />
                  <motion.button
                    whileHover={!isCommentDisabled ? { scale: 1.05 } : {}}
                    whileTap={!isCommentDisabled ? { scale: 0.95 } : {}}
                    onClick={handleSubmit}
                    disabled={isCommentDisabled}
                    className={`p-2 rounded-lg transition-all flex-shrink-0 ${
                      !isCommentDisabled ? 'text-indigo-500 hover:bg-indigo-50' : 'text-gray-300 cursor-not-allowed'
                    }`}
